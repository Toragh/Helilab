...
void P3p3_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P3p3_M)) {
     ...
     */
    P3p3_B.Sum4 = P3p3_B.Gain_e - P3p3_P.zerooutputconstant_Value;
  }
  ...
  P3p3_B.Gain1[4] = P3p3_P.Gain1_Gain * P3p3_B.Sum4;
  ...
  if (rtmIsMajorTimeStep(P3p3_M)) {
    ...
    for (i = 0; i < 6; i++) {
      rtb_TmpSignalConversionAtToFile[i] = P3p3_B.Gain1[i];
    }

    rtb_TmpSignalConversionAtToFile[6] = P3p3_B.Joystick_gain_x;
    rtb_TmpSignalConversionAtToFile[7] = P3p3_B.Joystick_gain_y;
    ...
    if (rtmIsMajorTimeStep(P3p3_M)) {
      {
        if (!(++P3p3_DW.ToFile_IWORK.Decimation % 1) &&
            (P3p3_DW.ToFile_IWORK.Count*9)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (((++P3p3_DW.ToFile_IWORK.Count)*9)+1 >= 100000000) {
              (void)fprintf(stdout,
     ...
     */
    P3p3_B.Gain1_o[0] = 0.0;
    P3p3_B.Gain1_o[0] += P3p3_P.P[0] * P3p3_B.Joystick_gain_x;
    P3p3_B.Gain1_o[0] += P3p3_P.P[2] * P3p3_B.Joystick_gain_y;
    P3p3_B.Gain1_o[1] = 0.0;
    P3p3_B.Gain1_o[1] += P3p3_P.P[1] * P3p3_B.Joystick_gain_x;
    P3p3_B.Gain1_o[1] += P3p3_P.P[3] * P3p3_B.Joystick_gain_y;
    P3p3_B.Constant2 = P3p3_P.Constant2_Value;
  }

  /* End of RateTransition: '<S4>/Rate Transition: x' */

  /* Step: '<S8>/Step' */
  if (P3p3_M->Timing.t[0] < P3p3_P.Step_Time) {
    rtb_Backgain = P3p3_P.Step_Y0;
  } else {
    rtb_Backgain = P3p3_P.Step_YFinal;
  }

  /* End of Step: '<S8>/Step' */

  /* Switch: '<S8>/Switch' */
  if (rtb_Backgain >= P3p3_P.Switch_Threshold) {
    rtb_Switch = P3p3_B.Gain1[5];
  } else {
    rtb_Switch = P3p3_B.Constant2;
  }
   ...
   */
  tmp[0] = P3p3_B.Gain1[2];
  tmp[1] = P3p3_B.Gain1[3];
  tmp[2] = rtb_Switch;
  ...
  for (i = 0; i < 2; i++) {
    rtb_Backgain = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Backgain += P3p3_P.K[(i_0 << 1) + i] * tmp[i_0];
    }

    rtb_Sum[i] = P3p3_B.Gain1_o[i] - rtb_Backgain;
  }
  ...
  if (rtmIsMajorTimeStep(P3p3_M)) {
     ...
     *  Constant: '<Root>/V_d+'
     */
    P3p3_B.Gain_o = P3p3_P.Gain_Gain_j * P3p3_P.V_d_Value;
  }

  /* Sum: '<Root>/Sum1' */
  rtb_Backgain = rtb_Sum[1] + P3p3_B.Gain_o;
  if (rtmIsMajorTimeStep(P3p3_M)) {
    /* Constant: '<Root>/V_s+' */
    P3p3_B.V_s = P3p3_P.v_star;
  }

  /* Sum: '<Root>/Sum' */
  rtb_Frontgain = P3p3_B.V_s + rtb_Sum[0];
  if (rtmIsMajorTimeStep(P3p3_M)) {
  }
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (rtb_Backgain + rtb_Frontgain) * P3p3_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (rtb_Frontgain - rtb_Backgain) * P3p3_P.Backgain_Gain;
  ...
  /* Sum: '<S5>/Sum1' */
  P3p3_B.Sum1 = P3p3_B.Gain1[2] - P3p3_B.Joystick_gain_x;

  /* Sum: '<S5>/Sum2' */
  P3p3_B.Sum2 = rtb_Switch - P3p3_B.Joystick_gain_y;
}
...
void P3p3_derivatives(void)
{
  ...
  _rtXdot->Integrator_CSTATE = P3p3_B.Sum1;
...
void P3p3_initialize(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(P3p3_M, "Error creating .mat file variables.mat");
      return;
    }
...
void P3p3_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "variables.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(P3p3_M, "Error closing MAT-file variables.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(P3p3_M, "Error reopening MAT-file variables.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 9, P3p3_DW.ToFile_IWORK.Count, "variables"))
      {
        rtmSetErrorStatus(P3p3_M,
...
RT_MODEL_P3p3_T *P3p3(void)
{
  ...
  /* External mode info */
  P3p3_M->Sizes.checksums[0] = (3181721150U);
  P3p3_M->Sizes.checksums[1] = (856586579U);
  P3p3_M->Sizes.checksums[2] = (1321590972U);
  P3p3_M->Sizes.checksums[3] = (3361491988U);
  ...
  P3p3_M->Sizes.numBlocks = (60);      /* Number of blocks */
  P3p3_M->Sizes.numBlockIO = (25);     /* Number of block outputs */
  P3p3_M->Sizes.numBlockPrms = (167);  /* Sum of parameter "widths" */
  return P3p3_M;
}
