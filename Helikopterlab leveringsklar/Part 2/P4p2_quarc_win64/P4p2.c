 ...
 * C source code generated on : Wed Oct 17 23:46:00 2018
 *
...
void P4p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
     ...
     */
    P4p2_B.Sum4 = P4p2_B.Gain_e - P4p2_P.zerooutputconstant_Value;
  }
  ...
  /* Gain: '<S3>/Gain1' */
  P4p2_B.Gain1[0] = P4p2_P.Gain1_Gain * P4p2_B.Gain;
  P4p2_B.Gain1[1] = P4p2_P.Gain1_Gain * P4p2_B.Gain_d;
  P4p2_B.Gain1[2] = P4p2_P.Gain1_Gain * P4p2_B.Gain_i;
  P4p2_B.Gain1[3] = P4p2_P.Gain1_Gain * P4p2_B.Gain_b;
  P4p2_B.Gain1[4] = P4p2_P.Gain1_Gain * P4p2_B.Sum4;
  P4p2_B.Gain1[5] = P4p2_P.Gain1_Gain * P4p2_B.Gain_dg;
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
    ...
    for (i = 0; i < 6; i++) {
      rtb_TmpSignalConversionAtToFile[i] = P4p2_B.Gain1[i];
    }

    rtb_TmpSignalConversionAtToFile[6] = P4p2_B.Joystick_gain_x;
    rtb_TmpSignalConversionAtToFile[7] = P4p2_B.Joystick_gain_y;
    for (i = 0; i < 6; i++) {
      rtb_TmpSignalConversionAtToFile[i + 8] = P4p2_B.Integrator[i];
    }
    ...
    if (rtmIsMajorTimeStep(P4p2_M)) {
      {
        if (!(++P4p2_DW.ToFile_IWORK.Decimation % 1) &&
            (P4p2_DW.ToFile_IWORK.Count*15)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (((++P4p2_DW.ToFile_IWORK.Count)*15)+1 >= 100000000) {
              (void)fprintf(stdout,
    ...
    P4p2_B.Constant2 = P4p2_P.Constant2_Value;
  }

  /* Step: '<S1>/Step' */
  if (P4p2_M->Timing.t[0] < P4p2_P.Step_Time) {
    unnamed_idx_0 = P4p2_P.Step_Y0;
  } else {
    unnamed_idx_0 = P4p2_P.Step_YFinal;
  }

  /* End of Step: '<S1>/Step' */

  /* Switch: '<S1>/Switch' */
  if (unnamed_idx_0 >= P4p2_P.Switch_Threshold) {
    P4p2_B.Switch = P4p2_B.Integrator[3];
  } else {
    P4p2_B.Switch = P4p2_B.Constant2;
  }
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
     ...
     */
    P4p2_B.Gain1_p[0] = 0.0;
    P4p2_B.Gain1_p[0] += P4p2_P.P[0] * P4p2_B.Joystick_gain_x;
    P4p2_B.Gain1_p[0] += P4p2_P.P[2] * P4p2_B.Joystick_gain_y;
    P4p2_B.Gain1_p[1] = 0.0;
    P4p2_B.Gain1_p[1] += P4p2_P.P[1] * P4p2_B.Joystick_gain_x;
    P4p2_B.Gain1_p[1] += P4p2_P.P[3] * P4p2_B.Joystick_gain_y;
  }
   ...
   */
  for (i = 0; i < 2; i++) {
    rtb_Sum[i] = P4p2_B.Gain1_p[i] - ((P4p2_P.K[i + 2] * P4p2_B.Integrator[1] +
      P4p2_P.K[i] * P4p2_B.Integrator[0]) + P4p2_P.K[i + 4] * P4p2_B.Switch);
  }
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
     ...
     */
    P4p2_B.Gain_o = P4p2_P.Gain_Gain_j * P4p2_P.Constant_Value;
  }

  /* Sum: '<Root>/Sum1' */
  rtb_Frontgain = rtb_Sum[1] + P4p2_B.Gain_o;
  if (rtmIsMajorTimeStep(P4p2_M)) {
    /* Constant: '<Root>/V_s*    ' */
    P4p2_B.V_s = P4p2_P.v_star;
  }

  /* Sum: '<Root>/Sum' */
  rtb_Backgain = P4p2_B.V_s + rtb_Sum[0];
  ...
  for (i = 0; i < 3; i++) {
    tmp[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp[i] += P4p2_P.C_est[3 * i_0 + i] * P4p2_B.Integrator[i_0];
    }
  }
   ...
   *  Gain: '<S4>/L'
   */
  unnamed_idx_0 = P4p2_B.Gain1[2] - tmp[0];
  unnamed_idx_1 = P4p2_B.Gain1[4] - tmp[1];
  unnamed_idx_2 = P4p2_B.Gain1[0] - tmp[2];
  for (i = 0; i < 6; i++) {
    /* Gain: '<S4>/L' incorporates:
     *  Sum: '<S4>/Sum3'
     */
    tmp_0[i] = P4p2_P.L[i + 12] * unnamed_idx_2 + (P4p2_P.L[i + 6] *
      unnamed_idx_1 + P4p2_P.L[i] * unnamed_idx_0);

    /* Gain: '<S4>/B_est' incorporates:
     *  Sum: '<S4>/Sum3'
     */
    tmp_1[i] = P4p2_P.B_est[i + 6] * rtb_Sum[1] + P4p2_P.B_est[i] * rtb_Sum[0];
  }
  ...
  for (i = 0; i < 6; i++) {
    unnamed_idx_0 = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      unnamed_idx_0 += P4p2_P.A_est[6 * i_0 + i] * P4p2_B.Integrator[i_0];
    }

    P4p2_B.Sum3[i] = (tmp_0[i] + tmp_1[i]) + unnamed_idx_0;
  }
   ...
   *  Sum: '<S2>/Add'
   */
  unnamed_idx_0 = (rtb_Frontgain + rtb_Backgain) * P4p2_P.Frontgain_Gain;
   ...
   *  Sum: '<S2>/Subtract'
   */
  unnamed_idx_0 = (rtb_Backgain - rtb_Frontgain) * P4p2_P.Backgain_Gain;
...
void P4p2_derivatives(void)
{
  ...
  for (i = 0; i < 6; i++) {
    _rtXdot->Integrator_CSTATE[i] = P4p2_B.Sum3[i];
  }
...
void P4p2_initialize(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(P4p2_M, "Error creating .mat file variables.mat");
      return;
    }
...
void P4p2_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "variables.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(P4p2_M, "Error closing MAT-file variables.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(P4p2_M, "Error reopening MAT-file variables.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 15, P4p2_DW.ToFile_IWORK.Count, "variables"))
      {
        rtmSetErrorStatus(P4p2_M,
...
RT_MODEL_P4p2_T *P4p2(void)
{
  ...
  /* External mode info */
  P4p2_M->Sizes.checksums[0] = (3928455584U);
  P4p2_M->Sizes.checksums[1] = (3798557980U);
  P4p2_M->Sizes.checksums[2] = (3044183647U);
  P4p2_M->Sizes.checksums[3] = (3855577670U);
  ...
  P4p2_M->Sizes.numBlocks = (64);      /* Number of blocks */
  P4p2_M->Sizes.numBlockIO = (26);     /* Number of block outputs */
  P4p2_M->Sizes.numBlockPrms = (246);  /* Sum of parameter "widths" */
  return P4p2_M;
}
