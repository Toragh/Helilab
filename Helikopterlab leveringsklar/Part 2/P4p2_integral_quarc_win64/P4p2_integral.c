...
void P4p2_integral_output0(void)       /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P4p2_integral_M)) {
     ...
     */
    P4p2_integral_B.Sum4 = P4p2_integral_B.Gain_e -
      P4p2_integral_P.zerooutputconstant_Value;
  }
  ...
  /* Gain: '<S3>/Gain1' */
  P4p2_integral_B.Gain1[0] = P4p2_integral_P.Gain1_Gain * P4p2_integral_B.Gain;
  P4p2_integral_B.Gain1[1] = P4p2_integral_P.Gain1_Gain * P4p2_integral_B.Gain_d;
  P4p2_integral_B.Gain1[2] = P4p2_integral_P.Gain1_Gain * P4p2_integral_B.Gain_i;
  P4p2_integral_B.Gain1[3] = P4p2_integral_P.Gain1_Gain * P4p2_integral_B.Gain_b;
  P4p2_integral_B.Gain1[4] = P4p2_integral_P.Gain1_Gain * P4p2_integral_B.Sum4;
  ...
  if (rtmIsMajorTimeStep(P4p2_integral_M)) {
    ...
    for (i = 0; i < 6; i++) {
      rtb_TmpSignalConversionAtToFile[i] = P4p2_integral_B.Gain1[i];
    }

    rtb_TmpSignalConversionAtToFile[6] = P4p2_integral_B.Joystick_gain_x;
    rtb_TmpSignalConversionAtToFile[7] = P4p2_integral_B.Joystick_gain_y;
    for (i = 0; i < 6; i++) {
      rtb_TmpSignalConversionAtToFile[i + 8] = P4p2_integral_B.Integrator[i];
    }
    ...
    if (rtmIsMajorTimeStep(P4p2_integral_M)) {
      {
        if (!(++P4p2_integral_DW.ToFile_IWORK.Decimation % 1) &&
            (P4p2_integral_DW.ToFile_IWORK.Count*15)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (((++P4p2_integral_DW.ToFile_IWORK.Count)*15)+1 >= 100000000) {
              (void)fprintf(stdout,
    ...
    P4p2_integral_B.Constant2 = P4p2_integral_P.Constant2_Value;
  }

  /* Step: '<S1>/Step' */
  if (P4p2_integral_M->Timing.t[0] < P4p2_integral_P.Step_Time) {
    unnamed_idx_0 = P4p2_integral_P.Step_Y0;
  } else {
    unnamed_idx_0 = P4p2_integral_P.Step_YFinal;
  }

  /* End of Step: '<S1>/Step' */

  /* Switch: '<S1>/Switch' */
  if (unnamed_idx_0 >= P4p2_integral_P.Switch_Threshold) {
    P4p2_integral_B.Switch = P4p2_integral_B.Integrator[3];
  } else {
    P4p2_integral_B.Switch = P4p2_integral_B.Constant2;
  }
  ...
  if (rtmIsMajorTimeStep(P4p2_integral_M)) {
     ...
     */
    P4p2_integral_B.Gain1_o[0] = 0.0;
    P4p2_integral_B.Gain1_o[0] += P4p2_integral_P.P[0] *
      P4p2_integral_B.Joystick_gain_x;
    P4p2_integral_B.Gain1_o[0] += P4p2_integral_P.P[2] *
      P4p2_integral_B.Joystick_gain_y;
    P4p2_integral_B.Gain1_o[1] = 0.0;
    P4p2_integral_B.Gain1_o[1] += P4p2_integral_P.P[1] *
      P4p2_integral_B.Joystick_gain_x;
    P4p2_integral_B.Gain1_o[1] += P4p2_integral_P.P[3] *
      P4p2_integral_B.Joystick_gain_y;
  }
  ...
  tmp[2] = P4p2_integral_B.Switch;
  tmp[3] = P4p2_integral_X.Integrator_CSTATE_m;
  ...
  for (i = 0; i < 2; i++) {
    unnamed_idx_0 = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      unnamed_idx_0 += P4p2_integral_P.K[(i_0 << 1) + i] * tmp[i_0];
    }

    rtb_Sum[i] = P4p2_integral_B.Gain1_o[i] - unnamed_idx_0;
  }
  ...
  if (rtmIsMajorTimeStep(P4p2_integral_M)) {
     ...
     */
    P4p2_integral_B.Gain_o = P4p2_integral_P.Gain_Gain_j *
      P4p2_integral_P.Constant_Value;
  }

  /* Sum: '<Root>/Sum1' */
  rtb_Backgain = rtb_Sum[1] + P4p2_integral_B.Gain_o;
  if (rtmIsMajorTimeStep(P4p2_integral_M)) {
    /* Constant: '<Root>/V_s*    ' */
    P4p2_integral_B.V_s = P4p2_integral_P.v_star;
  }

  /* Sum: '<Root>/Sum' */
  rtb_Frontgain = P4p2_integral_B.V_s + rtb_Sum[0];
  ...
  for (i = 0; i < 3; i++) {
    tmp_0[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_0[i] += P4p2_integral_P.C_est[3 * i_0 + i] *
   ...
   *  Gain: '<S4>/L'
   */
  unnamed_idx_0 = P4p2_integral_B.Gain1[2] - tmp_0[0];
  unnamed_idx_1 = P4p2_integral_B.Gain1[4] - tmp_0[1];
  unnamed_idx_2 = P4p2_integral_B.Gain1[0] - tmp_0[2];
  for (i = 0; i < 6; i++) {
    /* Gain: '<S4>/L' incorporates:
     *  Sum: '<S4>/Sum3'
     */
    tmp_1[i] = P4p2_integral_P.L[i + 12] * unnamed_idx_2 + (P4p2_integral_P.L[i
      + 6] * unnamed_idx_1 + P4p2_integral_P.L[i] * unnamed_idx_0);

    /* Gain: '<S4>/B_est' incorporates:
     *  Sum: '<S4>/Sum3'
     */
    tmp_2[i] = P4p2_integral_P.B_est[i + 6] * rtb_Sum[1] +
      P4p2_integral_P.B_est[i] * rtb_Sum[0];
  }
  ...
  for (i = 0; i < 6; i++) {
    unnamed_idx_0 = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      unnamed_idx_0 += P4p2_integral_P.A_est[6 * i_0 + i] *
        P4p2_integral_B.Integrator[i_0];
    }

    P4p2_integral_B.Sum3[i] = (tmp_1[i] + tmp_2[i]) + unnamed_idx_0;
  }
  ...
  if (unnamed_idx_0 > P4p2_integral_P.FrontmotorSaturation_UpperSat) {
    P4p2_integral_B.FrontmotorSaturation =
      P4p2_integral_P.FrontmotorSaturation_UpperSat;
  } else if (unnamed_idx_0 < P4p2_integral_P.FrontmotorSaturation_LowerSat) {
    P4p2_integral_B.FrontmotorSaturation =
      P4p2_integral_P.FrontmotorSaturation_LowerSat;
  } else {
    P4p2_integral_B.FrontmotorSaturation = unnamed_idx_0;
  }
   ...
   *  Sum: '<S2>/Subtract'
   */
  unnamed_idx_0 = (rtb_Frontgain - rtb_Backgain) * P4p2_integral_P.Backgain_Gain;

  /* Saturate: '<S5>/Back motor: Saturation' */
  if (unnamed_idx_0 > P4p2_integral_P.BackmotorSaturation_UpperSat) {
    P4p2_integral_B.BackmotorSaturation =
      P4p2_integral_P.BackmotorSaturation_UpperSat;
  } else if (unnamed_idx_0 < P4p2_integral_P.BackmotorSaturation_LowerSat) {
    P4p2_integral_B.BackmotorSaturation =
      P4p2_integral_P.BackmotorSaturation_LowerSat;
  } else {
    P4p2_integral_B.BackmotorSaturation = unnamed_idx_0;
  }
  ...
  /* Sum: '<S7>/Sum1' */
  P4p2_integral_B.Sum1 = P4p2_integral_B.Integrator[0] -
    P4p2_integral_B.Joystick_gain_x;

  /* Sum: '<S7>/Sum2' */
  P4p2_integral_B.Sum2 = P4p2_integral_B.Switch -
    P4p2_integral_B.Joystick_gain_y;
}
...
void P4p2_integral_derivatives(void)
{
  ...
  for (i = 0; i < 6; i++) {
    _rtXdot->Integrator_CSTATE[i] = P4p2_integral_B.Sum3[i];
  }

  /* End of Derivatives for Integrator: '<S4>/Integrator' */

  /* Derivatives for Integrator: '<S7>/Integrator' */
  _rtXdot->Integrator_CSTATE_m = P4p2_integral_B.Sum1;
...
void P4p2_integral_initialize(void)
{
...

  {
    ...
    P4p2_integral_X.Integrator_CSTATE_m = P4p2_integral_P.Integrator_IC_o;
...
void P4p2_integral_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (rt_WriteMat4FileHeader(fp, 15, P4p2_integral_DW.ToFile_IWORK.Count,
           "variables")) {
        rtmSetErrorStatus(P4p2_integral_M,
...
RT_MODEL_P4p2_integral_T *P4p2_integral(void)
{
  ...
  /* External mode info */
  P4p2_integral_M->Sizes.checksums[0] = (3085037992U);
  P4p2_integral_M->Sizes.checksums[1] = (258508123U);
  P4p2_integral_M->Sizes.checksums[2] = (4228591802U);
  P4p2_integral_M->Sizes.checksums[3] = (1927676939U);
...

  {
    ...
    P4p2_integral_B.Constant2 = 0.0;
    P4p2_integral_B.Switch = 0.0;
    P4p2_integral_B.Gain1_o[0] = 0.0;
    P4p2_integral_B.Gain1_o[1] = 0.0;
    P4p2_integral_B.Gain_o = 0.0;
    P4p2_integral_B.V_s = 0.0;
  ...
  P4p2_integral_M->Sizes.numBlockIO = (28);/* Number of block outputs */
  P4p2_integral_M->Sizes.numBlockPrms = (252);/* Sum of parameter "widths" */
