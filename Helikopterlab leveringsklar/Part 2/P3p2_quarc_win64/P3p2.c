 ...
 * C source code generated on : Wed Oct 17 23:30:05 2018
 *
...
void P3p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P3p2_M)) {
     ...
     */
    P3p2_B.Sum4 = P3p2_B.Gain_e - P3p2_P.zerooutputconstant_Value;
  }
  ...
  /* Gain: '<S2>/Gain1' */
  P3p2_B.Gain1[0] = P3p2_P.Gain1_Gain * P3p2_B.Gain;
  P3p2_B.Gain1[1] = P3p2_P.Gain1_Gain * P3p2_B.Gain_d;
  P3p2_B.Gain1[2] = P3p2_P.Gain1_Gain * P3p2_B.Gain_i;
  P3p2_B.Gain1[3] = P3p2_P.Gain1_Gain * P3p2_B.Gain_b;
  P3p2_B.Gain1[4] = P3p2_P.Gain1_Gain * P3p2_B.Sum4;
  P3p2_B.Gain1[5] = P3p2_P.Gain1_Gain * P3p2_B.Gain_dg;
  ...
  if (rtmIsMajorTimeStep(P3p2_M)) {
    ...
    for (i = 0; i < 6; i++) {
      rtb_TmpSignalConversionAtToFile[i] = P3p2_B.Gain1[i];
    }

    rtb_TmpSignalConversionAtToFile[6] = P3p2_B.Joystick_gain_x;
    rtb_TmpSignalConversionAtToFile[7] = P3p2_B.Joystick_gain_y;
    ...
    if (rtmIsMajorTimeStep(P3p2_M)) {
      {
        if (!(++P3p2_DW.ToFile_IWORK.Decimation % 1) &&
            (P3p2_DW.ToFile_IWORK.Count*9)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (((++P3p2_DW.ToFile_IWORK.Count)*9)+1 >= 100000000) {
              (void)fprintf(stdout,
     ...
     */
    P3p2_B.Gain1_o[0] = 0.0;
    P3p2_B.Gain1_o[0] += P3p2_P.P[0] * P3p2_B.Joystick_gain_x;
    P3p2_B.Gain1_o[0] += P3p2_P.P[2] * P3p2_B.Joystick_gain_y;
    P3p2_B.Gain1_o[1] = 0.0;
    P3p2_B.Gain1_o[1] += P3p2_P.P[1] * P3p2_B.Joystick_gain_x;
    P3p2_B.Gain1_o[1] += P3p2_P.P[3] * P3p2_B.Joystick_gain_y;
    P3p2_B.Constant2 = P3p2_P.Constant2_Value;
  }
  ...
  /* Switch: '<S8>/Switch' */
  if (rtb_Backgain >= P3p2_P.Switch_Threshold) {
    P3p2_B.Switch = P3p2_B.Gain1[5];
  } else {
    P3p2_B.Switch = P3p2_B.Constant2;
  }
   ...
   */
  for (i = 0; i < 2; i++) {
    rtb_Sum[i] = P3p2_B.Gain1_o[i] - ((P3p2_P.K[i + 2] * P3p2_B.Gain1[3] +
      P3p2_P.K[i] * P3p2_B.Gain1[2]) + P3p2_P.K[i + 4] * P3p2_B.Switch);
  }

  /* End of Sum: '<S5>/Sum' */
  if (rtmIsMajorTimeStep(P3p2_M)) {
    /* Constant: '<Root>/V_s+' */
    P3p2_B.V_s = P3p2_P.v_star;
  }

  /* Sum: '<Root>/Sum' */
  rtb_Backgain = P3p2_B.V_s + rtb_Sum[0];
  if (rtmIsMajorTimeStep(P3p2_M)) {
  }
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (rtb_Sum[1] + rtb_Backgain) * P3p2_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (rtb_Backgain - rtb_Sum[1]) * P3p2_P.Backgain_Gain;
...
void P3p2_initialize(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(P3p2_M, "Error creating .mat file variables.mat");
      return;
    }
...
void P3p2_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "variables.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(P3p2_M, "Error closing MAT-file variables.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(P3p2_M, "Error reopening MAT-file variables.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 9, P3p2_DW.ToFile_IWORK.Count, "variables"))
      {
        rtmSetErrorStatus(P3p2_M,
...
RT_MODEL_P3p2_T *P3p2(void)
{
  ...
  /* External mode info */
  P3p2_M->Sizes.checksums[0] = (4205165134U);
  P3p2_M->Sizes.checksums[1] = (3329370609U);
  P3p2_M->Sizes.checksums[2] = (574279012U);
  P3p2_M->Sizes.checksums[3] = (4134433586U);
  ...
  P3p2_M->Sizes.numBlockIO = (23);     /* Number of block outputs */
  P3p2_M->Sizes.numBlockPrms = (159);  /* Sum of parameter "widths" */
  return P3p2_M;
}
