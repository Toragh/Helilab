 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * heli_q8_part4_problem3(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(heli_q8_part4_problem3_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(heli_q8_part4_problem3_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://heli_q8_part4_problem3:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: heli_q8_part4_problem3 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (heli_q8_part4_problem3_DW.HILReadEncoderTimebase_Task, (t_clock)
           heli_q8_part4_problem3_P.HILReadEncoderTimebase_clock, 500.0, -1)
          ;
        if (result == 0) {
